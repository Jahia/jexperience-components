...
<c:choose>
    <c:when test="${currentNode.properties['wem:anonymizeProfile'].boolean or currentNode.properties['wem:activatePrivateBrowsing'].boolean}">
        ...
        <template:addResources type="javascript" resources="jquery.min.js,bootstrap-transition.js,bootstrap-modal.js"/>
        ...
        <div id="privacyModal_${currentNode.identifier}" class="modal hide fade">
            ...
            <div class="modal-body">
                ...
                <c:if test="${currentNode.properties['wem:anonymizeProfile'].boolean}">
                    ...
                    <button type="button" class="btn"
                            onclick="wem.anonymizeProfile(function() {location.reload()}, function(xhr) {$('#anonymizeError_${currentNode.identifier}').show(); console.error(xhr.responseText)})"
                            <c:if test="${renderContext.editMode}">disabled</c:if>>
                        ${anonymizeProfileButtonLabel}
                    </button>
                    <div id="anonymizeError_${currentNode.identifier}" class="alert alert-error">
                ...
                <c:if test="${currentNode.properties['wem:activatePrivateBrowsing'].boolean}">
                    <button id="privateBrowsing_${currentNode.identifier}"
                            ...
                            onclick="wem.togglePrivateBrowsing(function() {location.reload()}, function(xhr) {$('#privateBrowsingError_${currentNode.identifier}').show(); console.error(xhr.responseText)})"
                            <c:if test="${renderContext.editMode}">disabled</c:if>>
                        <c:if test="${renderContext.editMode}">
                            ${startPrivateBrowsingButton}
                        </c:if>
                    </button>
                    <div id="privateBrowsingError_${currentNode.identifier}" class="alert alert-error">
