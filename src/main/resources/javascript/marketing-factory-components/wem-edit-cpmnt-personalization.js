...
function componentPersonalizationConditionsInit(data) {
    if (data.isNewNode) {
        return $.parseHTML("<h1>Please click on save button to create your personalized component then add variants and you will be able to access this panel!</h1>");
    }
...
function componentPersonalizationConditionsDoSave(node) {
    ...
    if (angularScope && angularScope.$parent && angularScope.$parent.wemPersonalizationEditorCtrl) {
        angularScope.$parent.wemPersonalizationEditorCtrl.gwtSave(node);
    }
}
...
function componentPersonalizationConditionsDoValidate(validation) {
    var angularScope = customEditComponentPersoEngineGetAngularScope();
    if (angularScope && angularScope.$parent && angularScope.$parent.wemPersonalizationEditorCtrl) {
        angularScope.$parent.wemPersonalizationEditorCtrl.gwtValidation(validation);
    }
}
